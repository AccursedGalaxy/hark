...
- **Orchestration: head-session model — Phase 3 + live validation** (spec: `docs/orchestration-head.md`). Phase 1 (head role + `spawnHead`/`createWithHead` + head briefing + `hark` CLI + worker→head notification) and Phase 2 (`hark orch watch` long-poll + head-`DONE`→orchestration-complete + dashboard head surfacing) are **built and unit-tested**, dogfooded against a real repo with a fake spawner. Remaining: **(a)** validate the live keystroke loop against real Claude sessions under `HARK_ORCH_AUTONOMY=1` — head briefing fires only after trust clears, worker→head notifications wake the head, head metering populates; **(b)** Phase 3 polish: `hark pr` helper (push branch + `gh pr create --base`), decide on defaulting head-on; **(c)** the no-remote PR case (still-open question in the spec). NOTE: `hark` CLI requires `npm run build` before spawning (bin/hark imports `dist/`).
