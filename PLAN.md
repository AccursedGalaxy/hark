...
- **Orchestration: head-session model** (spec: `docs/orchestration-head.md`) — each orchestration gets one CC session as the *head*/foreman that talks to both the worker agents and the user; replaces the planned control-panel UI (diff viewer / PR button / conflict forms) with natural-language orchestration. **Decisions settled:** head spawns workers on demand (roles = palette), action surface = a thin `hark` CLI, head always auto-spawns, head in own worktree `hark/<orch>/head`. Folder-trust solved by pre-writing `~/.claude.json` (`hasTrustDialogAccepted`) per worktree before spawn (atomic merge; `-p`/`--dangerously-skip-permissions` don't fit our interactive-TUI model). Tool-permission posture = `--permission-mode auto` default (gated calls escalate via marker→notify path) + `bypassPermissions` opt-in. Reuses worktree isolation, store/event log, hardened send path, marker scan, metrics. Phase 1 = head role + spawnHead + head briefing + `hark` CLI (status/send/diff/spawn/brief) + worker→head marker notification. Ready to build on user greenlight.
