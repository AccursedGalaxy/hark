...
- **Blazing-fast session sync.** → board `fast-sync`. Both PRs implemented + verified 2026-06-10, in review: PR #43 (server quick wins: gzip, ETag/304, transcript parse cache, watcher-backed session index, session-list SSE push, offset-resumable streams) and PR #44 stacked on it (local-first: IndexedDB transcript cache + `?after=` deltas + lastUuid continuity check, windowed-tail rendering). Measured: 13MB transcript revisit = 30ms cache paint + 31-byte delta (was 4.5MB). Merge #43 → #44, deploy (rebuild + restart hark.service), then re-measure on phone. Plan: `~/.claude/plans/elegant-orbiting-kite.md`.
