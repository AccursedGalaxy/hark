...
- **Harness fidelity is the bottleneck.** → board `harness-fidelity`. Workers run far less reliably inside the harness than native Claude Code does, and nothing downstream (saturation, the board's payoff) matters until that's fixed. North-Star-adjacent goal: a hark worker should run as beautifully as Claude Code does natively. #1 is INSTRUMENT tool-call transport before any fix — we're blind to hark-drop vs platform-transient vs mis-read.
