...
- **Orchestration: persistent PM-head** (design in progress — spec: `docs/orchestration-head.md` §"Direction: the persistent PM-head"). Evolve the head from task-scoped executor into a persistent, project-scoped, conversational *product manager*: you open Claude Code in a project and chat normally (no upfront goal); the session owns `PLAN.md` as its externalized brain, tracks tasks/bugs/direction, and dispatches the existing head→worker engine only when you converge. **Decided:** (1) `PLAN.md`+inbox+issues = the PM's durable memory, so "head" is a role any session resumes by reading PLAN (solves the context-longevity problem); (2) the head is a **pure PM — never writes or runs code**, read-only on the working tree (enforced via a `PreToolUse` hook scoping `Edit`/`Write` to PLAN/coordination only), with git mutations routed to an integration worktree (`git -C`) / integrator worker / PRs-from-branches, and the final fast-forward landing left to the human. **Still open:** worker→head notification push-vs-pull (next), promotion mechanism (auto-announce vs `/head`), one-head-per-project always-on vs opt-in, coexist-vs-replace goal-first flow.
